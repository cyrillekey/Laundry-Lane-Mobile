...
class EmailVerifyPage extends StatefulHookConsumerWidget {
  const EmailVerifyPage({super.key});
  static const String routeName = "/email-verify";

  @override
  ConsumerState<EmailVerifyPage> createState() => _EmailVerifyState();
}

class _EmailVerifyState extends ConsumerState<EmailVerifyPage> {
                ...
                ProgressButton(
                  onPress: () async {
                    if (formKey.currentState?.saveAndValidate() == true) {
                      ...
                      AuthResponse response = await verifyEmailOtp(
                        user!.email!,
                        formData["otp"],
                      );
                      if (response.success) {
                        saveToken(response.token!, response.user!.id!);
                            ...
                            !canReset
                                ...
                                : () async {
                                  ...
                                  DefaultResponse response =
                                      await requestVerifyEmailOtp(email);
                                  startTimer();
                                  if (context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
